...
-t, --tty=false 分配 tty 设备，该可以支持终端登录
-m, --memory="" 指定容器的内存上限
-P, --publish-all=false 指定容器暴露的端口
-p, --publish=[] 指定容器暴露的端口
-u, --user="" 指定容器的用户
-v, --volume=[] 给容器挂载存储卷，挂载到容器的某个目录
--mount 给容器挂载存储卷，支持 type=bind,source=<host_path>,target=<container_path>
type：挂载类型，可以是 bind（绑定挂载）、volume（卷挂载）或 tmpfs（临时文件系统）
source：主机上的目录或文件路径
target：容器内的目录或文件路径
readonly：可选参数，指定挂载为只读模式
-w, --workdir="" 指定容器的工作目录

...
public:
  ...
  bool allowRequest() {
    ...
    while (!window.empty()) {
      if (now - window.front() >= intervalSeconds)
        window.pop();
      else
        break;
    }
...
public:
  LeakyBucket(int capacity, int rate)
      : capacity(capacity), rate(rate), water(0.0), lastUpdateTime(std::chrono::steady_clock::now()) {}
  ...
  bool addWater() {
    ...
    // 更新水位（使用浮点数计算）
    // 桶以每秒 rate 个请求的速率漏水，所以漏掉的水量是 elapsed_seconds * rate
    water = std::max(0.0, water - elapsed_seconds * rate);
...
private:
  int capacity;  // 桶容量（整数）
  int rate;      // 漏水速率（整数，单位：请求/秒）
  double water;  // 当前水位（浮点数，精确表示）
...
- 漏桶：输出速率恒定，不允许突发流量（即使桶未满，输出速率也是固定的）
- 令牌桶：允许突发流量（只要桶中有足够的令牌，就可以一次性处理多个请求）

2. 桶的含义：

- 漏桶：桶中存储的是请求（水），桶以固定速率漏水（处理请求）
- 令牌桶：桶中存储的是令牌，系统以固定速率添加令牌，请求需要获取令牌才能被处理
...
- API 速率限制，确保不会超过上游服务的承受能力
- 网络流量整形，确保输出流量平稳
- 需要严格保证处理速率不超过某值的场景

具体场景：

- 数据库写入限流：确保写入操作不会超过数据库处理能力
- 消息队列消费者：以固定速率处理消息，避免下游系统过载

### 令牌桶更适合的适用场景

- 突发流量处理，如 Web 服务器应对突然的访问高峰
- 需要利用系统空闲期积累处理能力的场景
- 允许短期超限但长期平均速率受限的应用

具体场景：

- Web API限流：允许用户短时间内突发请求，但长期平均速率受限
- 网络带宽控制：允许短暂超速传输，但长期平均速率符合合同规定

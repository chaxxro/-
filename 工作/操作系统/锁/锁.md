...
public:
    ...
    void lockWrite() {
        ...
        ++m_writeCount;
        // 等待条件：没有活跃的读线程和写线程
        m_writeCond.wait(ul, [this](){return !m_isWriting && m_readCount == 0});
        m_isWriting = true;
    }
  
    bool tryLockWrite(std::chrono::milliseconds timeout) {
        std::unique_lock<std::mutex> ul(m_lock);
        ++m_writeCount;
        // 等待条件：没有活跃的读线程和写线程
        bool flag = m_writeCond.wait(ul, timeout, [this](){return !m_isWriting && m_readCount == 0});
        if (flag) {
            m_isWriting = true;
        }
        return flag;
    ...
    void lockRead() {
        std::unique_lock<std::mutex> ul(m_lock);
        // 等待条件：没有写线程活动且没有等待的写线程
        m_readCond.wait(ul, [this](){return !m_isWriting && 0 == m_writeCount;});
        ++m_readCount;
    }
  
    bool tryLockRead(std::chrono::milliseconds timeout) {
        std::unique_lock<std::mutex> ul(m_lock);
        return read_cv_.wait_for(lock, timeout, [this]() {
            return !m_isWriting && 0 == m_writeCount;
        });
    }

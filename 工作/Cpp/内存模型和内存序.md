...
与其它内存序相比，`memory_order_relaxed` 内存序具有最少的同步开销，对于多线程递增计数是线程安全的

`std::shared_ptr` 的引用计数递增就是利用 `memory_order_relaxed` 的 `fetch_add` 实现的。但引用计数的递减需要更严格的内存顺序，通常使用 `memory_order_acq_rel` 确保在销毁对象之前，所有对对象的访问都已经完成
...
- A 线程中 `store()` 之前的内存操作（包括非原子变量和 relaxed 模型的读写操作）不允许重排至 `store()` 之后，所有之前的写内存操作完成后才能执行本条原子操作，但之前的写操作是允许乱序的，并且这些写入操作对线程 B 可见
- B 线程中 `load()` 之后的内存操作（读写操作）不允许重排至 `load()` 之前，所有后续的读内存操作都必须在本条原子操作完成后执行，但之后的读操作是允许乱序的
- `store` 之后的读写操作允许重排到 `store` 之前，`load` 之前的读写操作允许重排到 `load` 之后
...
`std::memory_order_acquire` 是要求后面所有内存操作都不得提前，`std::memory_order_consume` 是要求后面依赖于本次读的操作不能乱序
...
`std::atomic` 的操作都使用 `memory_order_seq_cst` 作为默认值。如果不确定使用何种内存访问模型，用 `memory_order_seq_cst` 能确保不出错
...
void read_x_then_y() {
  while (!x.load(std::memory_order_seq_cst))// x == false 就会死循环 
    ;
...
void read_y_then_x() {
  while (!y.load(std::memory_order_seq_cst))// y == false 就会死循环 
    ;
...
int main() {
  ...
  c.join();
  d.join();
  // 由于 memory_order_seq_cst 保证了线程可见性，所以一定不会触发
  assert(z.load() != 0);
}
```

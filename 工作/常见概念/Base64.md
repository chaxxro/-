...
1. 一个传输协议如果是基于 ASCII 文本的（如HTTP、SMTP、FTP的命令通道、Redis协议等），那么它就不能传输二进制流，如果要传输二进制流就得编码。常见的诸如 http 协议的 url 就是纯文本的，不能直接放二进制流。之所以不能直接传输二进制流，主要原因在于它们依赖于特定的控制字符来划分和解析数据。而二进制流是由任意字节序列组成的。一个字节可以是从 `0x00` 到 `0xFF` 的任何值，这意味着，二进制流中极有可能包含那些被文本协议视为特殊控制字符的字节值
